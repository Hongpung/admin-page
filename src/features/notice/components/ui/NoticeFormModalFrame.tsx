...
import { NOTICE_ACTION } from "../../constants";

type NoticeFormModalFrameProps = {
  isOpen: boolean;
  onDismiss: () => void;
  children: ReactNode;
};

export function NoticeFormModalFrame({
  isOpen,
  onDismiss,
  children,
}: NoticeFormModalFrameProps) {
  return (
    <Modal
      isOpen={isOpen}
      onClose={onDismiss}
      contentClassName="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-lg bg-white px-8 py-6 shadow-lg"
    >
      <button
        type="button"
        onClick={onDismiss}
        ...
        aria-label={NOTICE_ACTION.closeAriaLabel}
      >
